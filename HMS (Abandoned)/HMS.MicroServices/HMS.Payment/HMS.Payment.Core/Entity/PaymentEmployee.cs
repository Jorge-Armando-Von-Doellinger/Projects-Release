...
public sealed class PaymentEmployee : Payment
{
    ...
    {
        //Discounts.
        Amount = HourlySalary * HoursWorkedInMonth;
    }
    ...
    private void ValidateBenefits()
    {
        ...
        if (!Benefits.All(x => Enum.IsDefined(typeof(BenefitsEnum), x)))
            throw new ArgumentOutOfRangeException(nameof(Benefits), "Beneficios do funcionario são inválido!");
        if (TotalAmountOfBenefits <= 0)
            throw new PaymentInvalidException("Valor minimo dos beneficios não foram cumpridos");
    }

    private void ValidateIds()
    {
        if (string.IsNullOrWhiteSpace(EmployeeId))
            throw new ArgumentNullException(nameof(EmployeeId), "Identificador do funcionario não pode ser null");
    }
}