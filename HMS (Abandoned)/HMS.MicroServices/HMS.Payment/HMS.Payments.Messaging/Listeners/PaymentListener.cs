...
public sealed class PaymentListener : IMessageListener
{
    ...
    public PaymentListener(IServiceProvider provider, ChannelFactory channelFactory,
        IOptionsMonitor<MessagingSettings> settings)
    {
        _processor = provider
            .CreateAsyncScope()
            .ServiceProvider
            .GetRequiredKeyedService<IMessageProcessor>(MessageProcessorSelectorEnum.Payment.ToString());
    ...
    public async Task ListeningAsync(CancellationToken cancellationToken)
    {
        ...
        await channel.BasicConsumeAsync(_settings.GetPaymentQueue(), false, consumer, cancellationToken);
    }
}